"""
Typesense NL Search Implementation

This module provides search functionality using Typesense's native NL integration.
Instead of the decoupled middleware approach, this directly calls Typesense with
nl_query=true, allowing Typesense to call the middleware automatically.

Architecture:
    API → Typesense (nl_query=true) → Middleware (RAG classification) → Results

Benefits:
    - Single search call (no manual orchestration)
    - Works with any Typesense integration (Magento, Shopify, etc.)
    - Cleaner architecture
    - Middleware handles RAG classification transparently

Trade-offs:
    - No detailed RAG metadata in response (Typesense doesn't expose it)
    - Category confidence/reasoning not visible to API
    - Can't apply custom confidence thresholds
"""
